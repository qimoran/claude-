    ...
    <div className="h-full flex flex-col bg-claude-bg">
      ...
      <div className="px-3 py-1.5 border-b border-claude-border/50 flex items-center gap-1.5 bg-claude-surface/30" style={{ fontSize: `${settings.fontSizeUI}px` }}>
        ...
        <div className="ml-auto flex items-center gap-0.5">
          {/* Token 用量 */}
          {hasUsageStats && (
            <span className="text-[10px] text-claude-text-dim mr-2 hidden sm:inline" title={`对话: ${sessionUsage.messageCount} | 调用: ${sessionUsage.requestCount} | 输入: ${formatTokens(sessionUsage.totalInputTokens)} | 输出: ${formatTokens(sessionUsage.totalOutputTokens)} | 费用: ${formatCost(sessionUsage.totalCost, { unknownRequests: sessionUsage.unknownPriceRequestCount })} | 耗时: ${formatDuration(sessionUsage.totalDurationMs)}${sessionUsage.lastDurationMs > 0 ? ` (最近 ${formatDuration(sessionUsage.lastDurationMs)})` : ''} | 模型耗时: ${formatDuration(sessionUsage.modelTotalDurationMs)}${sessionUsage.lastModelDurationMs > 0 ? ` (最近 ${formatDuration(sessionUsage.lastModelDurationMs)})` : ''}`}>
